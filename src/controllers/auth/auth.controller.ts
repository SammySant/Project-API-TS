...
export default class AuthController{
    ...
    static async store (req: Request, res: Response){
        ...
        //Gera a hash da senha com o bcrypt - para não salvar a senha em texto puro
        user.password = bcrypt.hashSync(password, 10)
        await user.save()

        //Não retorna-se a hash da senha
    ...
    static async login (req: Request, res: Response){
        ...
        //Gera um refresh de token aleatório
    ...
    static async refresh (req: Request, res: Response){
        ...
        if(!token) return res.status(400).json({error: "Refresh token inválido!"})
